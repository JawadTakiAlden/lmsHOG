import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, IconButton, ListItemIcon, ListItemText, MenuItem, Tooltip, Typography } from '@mui/material'
...
const AllAccounts = () => {
    ...
    const {data , isError , isLoading , error , isRefetching , refetch} = useGetAccounts()
    ...
    const columns = useMemo(() => [
        ...
        editSelectOptions: [
            ...
            Cell : ({row}) => {
                return <Typography
                    sx={{
                        backgroundColor : row.original.type === 'admin' ? "#f56" : row.original.type === 'teacher' ? '#239' : '#91b',
                        color : 'white',
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="delete-account-area">
            are you sure you want to delete the account fro system this action cant be undo
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose} color='error' variant='outlined' sx={{borderRadius : '12px'}}>Disagree</Button>
          <LoadingButton
